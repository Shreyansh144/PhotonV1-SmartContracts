module photon_client_deployer::PhotonClientModule {
    ...
    struct ClientStore has key, copy, store {
        ...
        last_client_id: u64,  
        client_id_map: SimpleMap<u64, address>,                
    }
    ...
    // Resolve client address from seeds, aborting if not registered
    public fun is_client_registered(client_address: address):bool {
        exists<ClientRegistry>(client_address)
    }
    ...
    public entry fun initialize_client_store(admin: &signer) {
        ...
        if (!exists<ClientStore>(admin_addr)) {
            move_to(admin, ClientStore { 
                ...
                last_client_id: 0,
                client_id_map: simple_map::create()
            });
            move_to(admin, Admin { 
                owner: admin_addr
            });
    ...
    public entry fun register_client(
        ...
        maps.last_client_id = maps.last_client_id + 1;
        let last_id = maps.last_client_id;
        simple_map::add(&mut maps.client_id_map, last_id, client_address);
