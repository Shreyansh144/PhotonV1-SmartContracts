...
//     fun get_client_signer_address(): address acquires ClientStore {
//         let client_cap = &borrow_global<ClientStore>(@default_admin).cap;
...
//     //     assert!(admin_addr == @PhotonResourceAddress, error::invalid_argument(E_INVALID_OWNER));
...
//     //     if (!exists<ClientStore>(@PhotonResourceAddress)) {
...
//         seeds: vector<u8>,
//         isProtocol: bool
//     ) acquires ClientStore, ClientRegistryEvents {
...
//             created_at: now,
//             active: true,
//             coin_type: @PhotonResourceAddress, // Default coin type address, can be updated later
...
//     // ====== View client ======
//     public fun get_client(admin: &signer, client_seeds: vector<u8>): ClientRegistry acquires ClientStore {
...
//     public entry fun set_active(admin: &signer, client_seeds: vector<u8>, active: bool) acquires ClientStore, ClientRegistryEvents {
//         assert_admin(admin);
//         let admin_addr = signer::address_of(admin);
//         let maps = borrow_global<ClientStore>(@PhotonResourceAddress);
//         let client_addr_opt = simple_map::get(&maps.clientMap, &client_seeds);
//         if (!option::is_some(&client_addr_opt)) {
//             abort E_NOT_REGISTERED;
//         };
...
//     public entry fun set_kyc(admin: &signer, client_seeds: vector<u8>, verified: bool) acquires ClientStore, ClientRegistryEvents {
//         assert_admin(admin);
//         let admin_addr = signer::address_of(admin);
//         let maps = borrow_global<ClientStore>(admin_addr);
//         let client_addr_opt = simple_map::get(&maps.clientMap, &client_seeds);
...
//     public fun is_registered(admin: &signer, client_seeds: vector<u8>): bool acquires ClientStore {
//         assert_admin(admin);
//         let admin_addr = signer::address_of(admin);
//         let maps = borrow_global<ClientStore>(admin_addr);
...
//     public entry fun remove_client(admin: &signer, client_seeds: vector<u8>) acquires ClientStore {
//         assert_admin(admin);
//         let admin_addr = signer::address_of(admin);
//         let maps = borrow_global_mut<ClientStore>(admin_addr);
...
//     public fun get_client_resource_address(admin: &signer, client_seeds: vector<u8>): address acquires ClientStore {
//         assert_admin(admin);
//         let admin_addr = signer::address_of(admin);
//         let maps = borrow_global<ClientStore>(admin_addr);
//         let client_addr_opt = simple_map::get(&maps.clientMap, &client_seeds);
...
//     public fun is_protocol_client(admin: &signer, client_seeds: vector<u8>): bool acquires ClientStore {
//         assert_admin(admin);
//         let admin_addr = signer::address_of(admin);
//         let maps = borrow_global<ClientStore>(admin_addr);
//         let client_addr_opt = simple_map::get(&maps.clientMap, &client_seeds);
...
//     // ====== Get client balance ======
//     public fun get_client_balance<CoinType>(admin: &signer, client_seeds: vector<u8>): u128 acquires ClientStore {
//         assert_admin(admin);
//         let maps = borrow_global<ClientStore>(@PhotonResourceAddress);
//         let client_addr_opt = simple_map::get(&maps.clientMap, &client_seeds);
//         if (!option::is_some(&client_addr_opt)) {
