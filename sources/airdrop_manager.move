module airdrop_manager_deployer::airdrop_manager_module {
    ...
    public entry fun init_airdrop_manager(admin: &signer) {
        ...
        let (airdrop_resource_signer, airdrop_cap) = account::create_resource_account(admin, b"airdrop_manager_test_5");
    ...
    public entry fun admin_update_amount(
        admin: &signer,
    ...
    public entry fun batch_update_amounts(
        admin: &signer,
    ...
    public entry fun claim_airdrop(
        ...
        let already_claimed = if (simple_map::contains_key(&airdrop_manager.airdrop_claimed, &user_addr)) {
        ...
        let remaining_claimable = amount_per_user - already_claimed;
        ...
        *client_balance_ref = client_balance - requested_amount;
        ...
        if (simple_map::contains_key(user_claims, &client_address)) {
            ...
            *claimed_ref = already_claimed + requested_amount;
        } else {
    ...
    public entry fun claim_all_remaining(
        user: &signer,
        client_address: address,
    ) acquires AirdropManager, Capabilities, AdminStore {
        let user_addr = signer::address_of(user);
    ...
    #[view]
    public fun is_client_configured(client_address: address): bool acquires AirdropManager, AdminStore {
        let airdrop_manager = borrow_global<AirdropManager>(get_resource_address());
        simple_map::contains_key(&airdrop_manager.airdrop_amount_per_user, &client_address)
    }
    ...
    #[view]
    public fun has_fully_claimed(user_address: address, client_address: address): bool acquires AirdropManager, AdminStore {
        get_remaining_claimable(user_address, client_address) == 0
    }

    /// Get user's claim status for a client (claimed, remaining, total)
    #[view]
    public fun get_claim_status(user_address: address, client_address: address): (u64, u64, u64) acquires AirdropManager, AdminStore {
    ...
    /// Check if client is registered in PhotonClientModule
    #[view]
